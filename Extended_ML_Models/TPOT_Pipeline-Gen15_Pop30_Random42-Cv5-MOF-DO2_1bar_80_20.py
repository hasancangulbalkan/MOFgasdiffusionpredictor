...
# NOTE: Make sure that the outcome column is labeled 'target' in the data file
tpot_data = pd.read_excel('O2_extended model.xlsx', dtype=np.float64)
features = tpot_data.drop('log(D) 1bar(cm²/s)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['log(D) 1bar(cm²/s)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.005693695721261741
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=RidgeCV()),
    VarianceThreshold(threshold=0.0001),
    StackingEstimator(estimator=LinearSVR(C=0.001, dual=True, epsilon=0.01, loss="squared_epsilon_insensitive", tol=0.1)),
    ExtraTreesRegressor(bootstrap=False, max_features=0.5, min_samples_leaf=1, min_samples_split=3, n_estimators=100)
)
